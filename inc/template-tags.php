...
if ( ! function_exists( 'flagship_tailwind_posted_on' ) ) :
	...
	function flagship_tailwind_posted_on() {
		...
		$posted_on = sprintf(
			...
			esc_html_x( '%s', 'post date', 'flagship-tailwind' ),
			'<span class="font-bold font-sans-bold">' . $time_string . '</span>'
		);
...
if ( ! function_exists( 'flagship_tailwind_post_thumbnail' ) ) :
	...
	function flagship_tailwind_post_thumbnail() {
		...
		<?php else : ?>
			...
			<a class="border-none post-thumbnail hover:opacity-75" href="<?php the_permalink(); ?>" aria-hidden="true" tabindex="-1">
				<?php
					the_post_thumbnail(
						'medium_large',
						array(
